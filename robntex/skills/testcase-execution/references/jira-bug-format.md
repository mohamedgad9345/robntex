...
2. **Title style** — action-oriented, present tense, no filler:
   - ✅ "Eligibility check returns 500 for valid Emirates ID"
...
3. **Priority mapping** (inherit from the TC unless the defect is worse than the TC's severity):
   - Critical: blocks the whole flow, corrupts data, or breaks a regulator submission (NPHIES/Riayati/Shafafiya).
...
5. **Never include secrets** in the block — no tokens, no real Emirates IDs (mask as
   `784-XXXX-XXXXXXX-X`), no real patient names.
